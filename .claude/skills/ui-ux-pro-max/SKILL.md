...
The search script lives inside this skill's own directory, not in the app's source. Run it from the repository root, where the paths below resolve; from anywhere else, use the absolute path to `.claude/skills/ui-ux-pro-max/scripts/search.py`:
